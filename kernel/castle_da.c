...
static void castle_da_merge_dealloc(struct castle_da_merge *merge, int err)
{
    ...
    if(!err)
    {
        ...
        {
            castle_printk("Empty merge at level: %u\n", merge->level);
                                                ...
                                                struct castle_merge_token *token)
{
    ...
    BUG_ON(read_can_lock(&da->lock)); /* We should not be able to read lock,
                                         if already write locked. */
...
static void __castle_da_level0_modified_promote(struct work_struct *work)
{
    ...
    for (cpu_index = 0; cpu_index < castle_double_array_request_cpus(); cpu_index++)
    {
        ...
        if (atomic64_read(&ct->item_count) != 0)
        {
            castle_printk("Promote for DA 0x%x level 0 RWCT seq %u (has %ld items)\n",
                    da->id, ct->seq, atomic64_read(&ct->item_count));
            __castle_da_rwct_create(da, cpu_index, 0 /*in_tran*/);
        }
                                                     ...
                                                     int level)
{
    ...
    {
        castle_printk("Could not allocate a CT because of sequence # overflow.\n");
        castle_free(ct);
        return NULL;
    }
    atomic_set(&ct->ref_count, 1);
